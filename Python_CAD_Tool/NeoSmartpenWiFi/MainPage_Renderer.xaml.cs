...

{
...

    {
        ...
        private async void ProcessDot(Dot dot)

        {
            Debug.WriteLine("Pressure: " + DotPressurePrint(dot) + "Position: " + DotPositionPrint(dot));
            ...
            {
                stopWatch.Stop();
                //Debug.WriteLine("end time: " + stopWatch.ElapsedMilliseconds.ToString());
            }
            else { i++; }
            ...
            // What if here we were to send the dot information to the server?
            String dotData = DotPositionPrint(dot) + "/" + DotPressurePrint(dot);
            //await SendMessageToRemoteServer("ws://192.168.191.2:8080", dotData);
            await SendMessageToTCPServer("ws://192.168.191.2:8080", dotData);
            //Debug.WriteLine(DotPositionPrint(dot));
            //Debug.WriteLine(DotPressurePrint(dot));
            //Debug.WriteLine(DotTiltPrint(dot));

            if (dot.DotType == DotTypes.PEN_UP)
            {
               // await SendMessageToRemoteServer("ws://localhost:8080/WebsocketHttpListenerDemo", "pen-up");
                // 최종 획을 그린다.
        ...
        private String DotTiltPrint(Dot dot)
        {
            return "tilt: " + dot.TiltX.ToString() + " " + dot.TiltY.ToString() + " twist: " + dot.Twist.ToString();
        }

        private String DotInfo(Dot dot)
        {
            if (DotPressureHigh(dot))
            {
                return "high " + DotPositionPrint(dot);
            }
            else { return "low " + DotPositionPrint(dot); }
        
        }

        // returns true if the dot's pressure is above the threshold
        private bool DotPressureHigh(Dot dot)
        {
            if (dot.Force >= 400)
            {
                return true;
        ...
        private String DotPressurePrint(Dot dot)
        {
            // Keep a fixed number (4) of digits of the float to fill the fixed-length buffer in the tcp socket
            string format = "{0:0000.00}";
            string pressure = string.Format(format, dot.Force);
            return pressure;
        }

        private string DotPositionPrint(Dot dot)
        {
            string format = "{0:000.00}";
            string X = string.Format(format, dot.X);
            string Y = string.Format(format, dot.Y);

            return X + "," + Y;
        }

        private String DotToString(Dot d)
        {
