...
class ControlThread(QThread):
        ...
        def DriveControl(self):
                ...
                if(self.activeControl==True):
                        ...
                        if (self.mode=='Rect' and self.state==2):
                                if(distance[1]>0):
                                        GPIO.output(DIR1,1)
                                ...
                                if (self.currPenCoord[0]+self.x*1.7<self.target[0] and self.currPenForce>10):
                                        GPIO.output(DIR2,0)
                                        self.pwmLib.pwmWrite(pwm_Pin2,int(np.round(offset+350)))
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin2,0)
                                        self.x=0
                        ...
                        if (self.mode=='Rect' and self.state==3):
                                if(distance[0]>0):
                                        GPIO.output(DIR2,0)
                                ...
                                if (self.currPenCoord[1]+self.y*1.45>self.target[1] and self.currPenForce>10):
                                        GPIO.output(DIR1,0)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+300))) #going up 
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin1,0)
                        ...
                        if (self.mode=='Rect' and self.state==4):
                                if(distance[1]<0):
                                        GPIO.output(DIR1,0)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+300))) #going up
