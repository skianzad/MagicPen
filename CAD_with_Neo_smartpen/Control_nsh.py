        ...
        def DriveControl(self):
                ...
                if(self.activeControl==True):
                        ...
                        if ((self.mode=='Rect' or self.mode=='RectP') and self.state==2):
                                if(distance[1]>0):
                                        GPIO.output(DIR1,1)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+200))) #going down
                                        
                                elif(distance[1]<-10):
                                        GPIO.output(DIR1,0)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+300))) #going up
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin1,0)
                                        
                                if (self.currPenCoord[0]+self.x*1.7<self.target[0] and self.currPenForce>10):
                                        GPIO.output(DIR2,0)
                                        self.pwmLib.pwmWrite(pwm_Pin2,int(np.round(offset+300))) #pushing right
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin2,0)
                                        self.x=0
                        ...
                        if ((self.mode=='Rect' or self.mode=='RectP') and self.state==3):
                                if(distance[0]>0):
                                        GPIO.output(DIR2,0)
                                        self.pwmLib.pwmWrite(pwm_Pin2,int(np.round(offset+300))) #going right
                                elif(distance[0]<-10):
                                        GPIO.output(DIR2,1)
                                        self.pwmLib.pwmWrite(pwm_Pin2,int(np.round(offset+300))) #going left
                                ...
                                if (self.currPenCoord[1]+self.y*1.45>yline and self.currPenForce>10):
                                        GPIO.output(DIR1,0)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+350)))
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin1,0)
                                        self.y=0 
                        ...
                        if ((self.mode=='Rect' or self.mode=='RectP')and self.state==4):
                                ...
                                if (self.currPenCoord[0]+self.x*1.7>self.target[0] and self.currPenForce>10):
                                        GPIO.output(DIR2,1)
                                        self.pwmLib.pwmWrite(pwm_Pin2,int(np.round(offset+350))) # going left
                                else:
                                        self.pwmLib.pwmWrite(pwm_Pin2,0)
                                        self.x=0
                        ...
                        if (self.state==5):
                                ...
                                if (self.currPenCoord[1]+self.y*1.45<yline and self.currPenForce>10):
                                        GPIO.output(DIR1,1)
                                        self.pwmLib.pwmWrite(pwm_Pin1,int(np.round(offset+350))) #pushing down
                ...
                #making a Triangle                
                        ...
                        offset   =      50
                        #if(self.currPenForce>100):       #variable pressure gain
                                #gain=400*abs((180-self.currPenForce))/80
                                #self.x=0
                                #self.y=0
                                #self.lineEndCoord=self.currPenCoord
                                #offset=0
