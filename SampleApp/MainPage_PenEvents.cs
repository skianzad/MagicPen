...
namespace SampleApp
{
    ...
    public sealed partial class MainPage
	{
        ...
        private async void InstantiateWS()
        {   /*
            Debug.WriteLine("WS Instantiating");
            ...
            try
            {
                Task connectTask = this.messageWebSocket.ConnectAsync(new Uri("ws://Localhost:8080/WebsocketHttpListenerDemo")).AsTask();
               // connectTask.ContinueWith(_ => SendMessage(messageWebSocket, "Hello, World!"));
            }
            catch (Exception ex)
            {
            ...
            {
                if (i % 100 ==0) { 
                await SendMessageToRemoteServer("ws://localhost:8080/WebsocketHttpListenerDemo", i.ToString() + " " + i.ToString());
                //await SendMessageToRemoteServer("ws://localhost:8080/WebsocketHttpListenerDemo", "100 100");
            }
            } */
         ...
         **/
        private async void SendMessage(Windows.Networking.Sockets.MessageWebSocket ws, string message)
        {
        ...
        private async Task SendMessageToRemoteServer(string url, string message)
        {
           ...
           // Debug.WriteLine("Sending msg: " + message);
            try
            {
                ...
                //Debug.WriteLine("webSocket is not null");
		...
		{
            Debug.Write("InitPenClient called");
            ...
            /*** also instantiating websocket stuff here ***/

           // InstantiateWS();
            //Debug.Write("WS instantiated");
...
#region Watcher Event
		...
		{
            Debug.Write("MClient_onUpdatePenController called");
		...
		{
			//Debug.WriteLine("Watcher finished");
