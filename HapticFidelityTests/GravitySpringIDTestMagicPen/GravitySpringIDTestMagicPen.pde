...
void setup() {
  ...
  //insertObject("box", 4); // arguments: "box" "bump" "triangle" "circle"
    //<>//
...
void draw() {
  ...
  //}
   ...
   if (bump != null) {
 //  rect(bump.position.x-bump.rampWidth/2, bump.position.y-bump.rampLength, bump.rampWidth, bump.rampLength*2);
   }
...
void onTickEvent(CountdownTimer t, long timeLeftUntilFinish){
  ...
  f_ee.add(getGravityForce()); //adds gravity force
...
void mouseReleased(){
  ...
  void keyPressed() {
  if (key == TAB) { //save the data in the text file
